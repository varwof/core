...
//
// Encoding specification (consistent with types Capability.FullID()):
...
// FullID returns the full capability identifier (schemeId:capacityId), semantically identical to types Capability.FullID().
...
// Field order (consistent with types): reason → requestedLifetime → timestamp → nonce →
...
// spec P1-A-12). SHA-2/SHA-3 + SM3 (built-in pure Go implementation in types, C1) are validated
